...
void main(void){
    ...
    //Para realizar el cilindro se utilizo trigonometria,el eje X y Z dados por la circufenrecia y el eje Y paralelo al perpendicular al plano
    //nuevamente se utilizaron constantes para lograr lo pedido 
    position=vec3(0.5*cos(2.0*PI*uv.x),-1.0*cos(2.0*PI*uv.y*0.5),0.5*sin(2.0*PI*uv.x));
