...
void main(void){
    ...
    //se identifica el agua 
     if (textureColor.y < textureColor.z) {
                //se modifica la diferecia de las alturas entre la tirra y el agua 
                position = position * 0.94;
                //se genera el las olas econ respecto a x y a y con las contantes determinadas
                position+=normal*(1.0+sin(uv.x*10.0*PI+time*30.0))*0.02; 
                position+=normal*(1.0+sin(uv.y*10.0*PI+time*30.0))*0.02; 
            }
